...
int main(int argc, char** argv)
{
	...
	VCD outVcd(file);
	outVcd.removeAllVars(); // actually only keeps header info
	
	int i = 0;
	while (true) {
		int offset = atoi(line.substr(pos + 1).c_str());
		//cout << file << "\t" << offset << "\n";

		char newId = outVcd.getFreeId();
		outVcd.addVar("RAS" + to_string(++i), newId, 1);
		VCD vcd(file);
		char id = vcd.getId("RAS"); assert(id);
		vector<char> v(1, id);
		for (auto& p : vcd.getFilteredEvents(v)) {
			outVcd.addChange(offset + p.first.first, newId, p.second);
		}
	...
	for (int i = 1; i < argc; ++i) {
		...
		vector<char> v(1, id);
		for (auto& p : vcd.getFilteredEvents(v)) {
			outVcd.addChange(p.first.first, newId, p.second);
		}
	}
