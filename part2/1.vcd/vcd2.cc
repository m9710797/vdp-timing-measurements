...
private:
    void parseVar();
    string getTillEnd();
    void processValueLine(const string& line, int& time);
...
string Tokenizer::getLine()
{
    string line;
    getline(in, line);
    if (!in.good() && !in.eof()) throw TokenizerEnd();
    if (!in.good() && in.eof()) throw TokenizerEnd();
    return line;
}
    ...
    while (true) {
        ...
        if (line == "$dumpvars") {
            while (true) {
                string dumpLine;
                try {
                    dumpLine = tokenizer.getLine();
                } catch (TokenizerEnd&) {
                    break;
                }
                if (dumpLine == "$end") break;
                if (dumpLine.empty()) continue;
                processValueLine(dumpLine, time);
            }
            continue;
        }
...
void VCDParser::processValueLine(const string& line, int& time)
{
    ...
    if (!tokens[0].empty() && tokens[0][0] == '#') {
        if (tokens[0].size() > 1) {
            time = atoi(tokens[0].substr(1).c_str());
        }
        start = 1;
    }
    ...
    for (size_t i = start; i < tokens.size(); ++i) {
        ...
        if (token[0] == 'b' || token[0] == 'B') {
            ...
            string bits = token.substr(1);
            char id = tokens[i + 1][0];
            int val = 0;
            for (char c : bits) {
                if (c == '1') {
                    val = (val << 1) | 1;
        ...
        if (token.size() >= 2) {
            ...
            if (token[0] == '1') {
                ...
                val = 0;
            } else if (token[0] == 'x' || token[0] == 'X' || token[0] == 'z' || token[0] == 'Z') {
                val = -1;
            }
            vcd.addChange(time, id, val);
        }
...
void VCD::dump()
{
    ...
    for (auto& p : vars) {
        char id = p.first;
        auto& var = p.second;
        cout << "$var wire " << var.size << " " << id << " " << var.name << " $end\n";
    }
...
void print(int time, int rw, int addr, int VDS, bool highUsed)
{
	...
	cout << dec << setfill(' ') << setw(5) << time << ": "
	     << (rw ? " R" : " W")
	     << (highUsed ? "b" : ".")
	     << (VDS ? "." : "v")
	     << " 0x" << hex << setfill('0') << setw(5) << addr2
	     << endl;
}
void printRAS(int time)
{
	cout << dec << setfill(' ') << setw(5) << time << ":" << endl;
}
void decode(const VCD& vcd)
{
...
{
    if (argc < 2) {
        cerr << "usage: " << argv[0] << " <file.vcd>" << endl;
        return 1;
    }
